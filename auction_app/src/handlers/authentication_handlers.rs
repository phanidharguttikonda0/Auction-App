...
use crate::models::authentication_models::{Authentication, Authorization, SignUp};
...
pub async fn login(State(state): State<AppState>, Form(login_data): Form<Authentication>) -> Json<Result<Authorization, String>> {
	...
	let result = sqlx::query_as::<_, Authentication>("Select mail_id,password from users where mail_id=($1) and password=($2)")
	.bind(&login_data.mail_id)
	.bind(&login_data.password)
	.fetch_one(&database_connection).await ;
	match result {
		Ok(res) => {
			println!("Result for login was {:#?}", res) ;
			let authorization_header = generate_authorization_header(login_data.mail_id) ;
...
pub async fn sign_up(State(state): State<AppState>,Form(new_user): Form<SignUp>) -> Json<Result<Authorization, String>> {
	...
	let database_connection = state.database_connection ;
	let result = sqlx::query("insert into users (username,mail_id, password) values ($1,$2,$3)")
	...
	match result {
	    Ok(res) => {
	    	...
	    	let authorization_header = generate_authorization_header(new_user.mail_id) ;
			Json::from(Ok(Authorization::new(authorization_header)))
	    },
	    Err(err) => {
